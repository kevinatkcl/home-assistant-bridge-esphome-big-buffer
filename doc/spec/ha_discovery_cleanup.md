...
`destroy()` MUST:
- If `device_id` is NULL, skip unsubscribe and zero the struct immediately (safe for partial init)
- If `subscribed` is true and `mqtt_client` is not NULL, unsubscribe from the wildcard topic
...
`run()` is the main entry point called each loop iteration. It drives the state machine through subscription, discovery, flush, and completion phases.
...
**Filtering:**
1. **Config topics only:** Skip if topic length < 7 or doesn't end with `/config`. Non-config topics (e.g., `/availability`) are ignored.
2. **Echo detection:** Skip if `payload_len == 0`. Empty payloads are our own echo from a previous clear operation.
...
2. If `queue_count == 0`: exit critical section, return 0
3. Copy the first topic string to a local 256-byte stack buffer (before compacting, as `memmove` invalidates pointers into `topic_buf`)
4. Calculate bytes consumed: `strlen(topic_buf) + 1`
5. Safety clamp: if `consumed > queue_write_pos`, clamp to `queue_write_pos`
6. Compact: `memmove` remaining data to front of `topic_buf`
7. Decrement `queue_write_pos` by `consumed`, decrement `queue_count`, increment `pass_removed_count`
8. Read `remaining = queue_count` while still in critical section
9. Exit critical section
10. Publish empty retained payload: `mqtt_client_publish_raw(mqtt_client, topic, "", 0, true)`
11. Log debug: "Removed old topic: {topic}"
12. Return `remaining` (number of topics still in queue)
