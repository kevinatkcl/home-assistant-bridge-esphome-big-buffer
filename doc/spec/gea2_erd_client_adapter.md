...
- Translate GEA2 request IDs to GEA3 request IDs (uint16_t cast).
- Re-publish GEA2 activity events as GEA3 activity events via the adapter's own `tiny_event`.
- Report that subscriptions are not supported (`subscribe()` always returns `false`; `retain_subscription()` is a no-op).
...
### `adapter_read(request_id, address, erd)`
...
### `adapter_write(request_id, address, erd, data, data_size)`
...
### `adapter_subscribe(address)`

Always returns `false`. GEA2 does not support subscriptions; callers should fall back to polling.

### `adapter_retain_subscription(address)`

No-op. Returns `false`. GEA2 does not support subscriptions.
...
GEA2 uses `tiny_gea2_erd_client_request_id_t` (uint16_t). GEA3 uses `tiny_gea3_erd_client_request_id_t` (uint16_t). The adapter casts the GEA2 ID to the GEA3 type with no transformation — both are `uint16_t`. This is safe because:
...
2. **Request ID cast assumes compatible types:** The adapter casts `uint16_t` GEA2 request IDs to GEA3 request IDs. If either type changes in the future, the adapter must be updated.
