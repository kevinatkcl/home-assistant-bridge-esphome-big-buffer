...
| Field | Description |
|-------|-------------|
| `interface` | `i_tiny_uart_t` vtable pointing to the static `api` |
...
Writes the byte to the UART via `uart->write_byte(byte)` and sets `sent = true`. The `send_complete_event` is not published here — it is deferred to the next `poll()` call.
...
| `i_tiny_uart.h` | Interface from tiny-gea-api |
| `tiny_event.h` | Event pub/sub system |
| `tiny_timer.h` | Periodic polling timer |
| `tiny_utils.h` | Utility functions |
