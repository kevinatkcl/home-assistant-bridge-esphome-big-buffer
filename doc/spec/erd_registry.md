...
**Behavior:**
- If `erds` is null or `count` is zero: returns immediately; filtering remains disabled. This prevents an empty set from silently suppressing all publishes.
...
3. **Empty valid set disables filtering:** If `set_valid_erds()` receives a null pointer or zero count, `valid_erds_ready_` remains `false` and `is_valid()` returns `true` for all ERDs.
