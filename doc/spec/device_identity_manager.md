...
|-----------|-------------|
| `configured_id` | Optional pre-configured device ID from YAML. If non-empty, it is stored but does not skip the ERD read sequence. |
| `erd_client` | GEA3 ERD client interface used to queue reads. |
| `host_address` | The appliance's GEA bus address for read requests. |

`init()` stores all parameters, resets state to `READING_APPLIANCE_TYPE`, and immediately queues the first ERD read (0x0008). A pre-configured device ID is logged at info level but does not alter the read sequence.
...
|-------|-------------|------------|------------|
| `READING_APPLIANCE_TYPE` | ERD 0x0008 | → `READING_MODEL_NUMBER` | Retry 0x0008 |
| `READING_MODEL_NUMBER` | ERD 0x0001 | → `READING_SERIAL_NUMBER` | Retry 0x0001 |
| `READING_SERIAL_NUMBER` | ERD 0x0002 | Assemble ID → `COMPLETE` | Retry 0x0002 |
| `COMPLETE` | — | — | — |

### 3.2 State Diagram

```
READING_APPLIANCE_TYPE (0x0008)
  ├─ read completed → store appliance_type_, → READING_MODEL_NUMBER
  └─ read failed    → retry 0x0008

READING_MODEL_NUMBER (0x0001)
  ├─ read completed → store model_number_, → READING_SERIAL_NUMBER
  └─ read failed    → retry 0x0001

READING_SERIAL_NUMBER (0x0002)
  ├─ read completed → store serial_number_, assemble generated_device_id_, → COMPLETE
...
- **Sanitized model:** `sanitize_for_mqtt_topic_(model_number_)` — raw model number bytes converted to string, trailing nulls trimmed, then sanitized.
- **Sanitized serial:** `sanitize_for_mqtt_topic_(serial_number_)` — raw serial number bytes converted to string, trailing nulls trimmed, then sanitized.
...
Returns `configured_device_id_` if non-empty; otherwise returns `generated_device_id_`.
...
class DeviceIdentityManager {
    ...
    std::string configured_device_id_;           // Pre-configured ID from YAML (if any)
    std::string generated_device_id_;            // Auto-generated ID: type_model_serial
    uint8_t appliance_type_;                     // Raw appliance type byte (from ERD 0x0008)
    std::string model_number_;                   // Raw model number string (from ERD 0x0001)
    std::string serial_number_;                  // Raw serial number string (from ERD 0x0002)
...
After `on_erd_read_completed()` transitions to `COMPLETE`, the bridge signals the startup HSM via `tiny_hsm_send_signal(signal_device_id_complete, nullptr)`.
