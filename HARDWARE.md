...
- **No PSRAM.** All data structures, buffers, and task stacks must fit in the 320 KB of on-chip SRAM shared with the OS and ESPHome framework. The MQTT publisher task therefore uses a statically allocated 4 KB stack (`xTaskCreateStaticPinnedToCore` on dual-core, `xTaskCreateStatic` on single-core); this accommodates the ESPHome/IDF MQTT call chain under queue pressure. ERD data is hex-encoded into fixed-size buffers.
