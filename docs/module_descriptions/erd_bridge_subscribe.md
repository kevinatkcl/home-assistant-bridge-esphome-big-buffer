...
sub_state_top (parent state — handles publication signals globally)
  ...
  │    │   (subscription_failed increments subscribe_failure_count; if >= 3, transitions to state_failed)
  │    ├─ subscription_host_came_online: clear ERD set, then attempt subscribe()
  │    ├─ subscription_added_or_retained → state_subscribed
  │    └─ exit: disarm timer
  │
  ├─ state_subscribed
  │    ├─ entry: arm periodic timer (30 s retention), arm quiet timer (2 s)
  │    ├─ timer_expired: retain subscription
  │    ├─ subscription_host_came_online → state_subscribing
  │    ├─ quiet_period_expired: if no ERDs published → state_failed, else → state_steady
  │    ├─ subscription_publication_received: if new ERD, restart quiet timer (stay in subscribed)
  │    └─ exit: disarm quiet timer
  │
  ├─ state_steady
  │    ├─ entry: disarm quiet timer, set current_state to steady
  │    ├─ timer_expired: retain subscription (handled by parent)
  │    ├─ subscription_host_came_online → state_subscribing
  │    ├─ subscription_publication_received: if new ERD, transition back to state_subscribed
  │    └─ exit: (retention timer persists)
  │
  └─ state_failed
       ├─ entry: disarm all timers, log "Subscription not supported by appliance"
       └─ Terminal state — no automatic recovery
```

Shared signals (handled in `erd_bridge_common.h`):
- `signal_subscription_publication_received` — publish ERD value to ERD cache
- `signal_quiet_period_expired` — transitions to steady or failed based on whether any ERDs were published
...
|-------|------|-------------|
| `quiet_timer` | `tiny_timer_t` | One-shot timer for the quiet period (2 s). Arms on entry to `state_subscribed`; fires `signal_quiet_period_expired`. |
| `subscribe_failure_count` | `uint8_t` | Consecutive subscribe failure counter. Incremented on each `signal_subscription_failed`. Transitions to `state_failed` when reaching 3. Reset on successful subscription. |
| `current_state` | `subscription_state_t` | Current HSM state as an enum for external diagnostics. |
