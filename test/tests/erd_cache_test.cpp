...
static erd_cache_entry_t* find_entry(erd_cache_t* cache, tiny_erd_t erd, uint8_t board_address)
{
  uint16_t iter = 0;
  erd_cache_entry_t* e;
  while ((e = erd_cache_get_next_entry(cache, &iter)) != NULL) {
    if (e->erd == erd && e->board_address == board_address) return e;
  }
  return NULL;
}
...
{
  uint8_t data[] = { 0xAB };
  bool result = erd_cache_update(&cache, 0x0001, 0xFF, data, 1);
  CHECK_TRUE(result);
  CHECK_EQUAL(1, erd_cache_get_count(&cache));

  erd_cache_entry_t* entry = find_entry(&cache, 0x0001, 0xFF);
  CHECK(NULL != entry);
...
TEST(erd_cache, insert_4_bytes)
{
  ...
  bool result = erd_cache_update(&cache, 0x0010, 0xFF, data, 4);
  CHECK_TRUE(result);

  erd_cache_entry_t* entry = find_entry(&cache, 0x0010, 0xFF);
  CHECK(NULL != entry);
...
TEST(erd_cache, insert_5_bytes)
{
  ...
  bool result = erd_cache_update(&cache, 0x0020, 0xFF, data, 5);
  CHECK_TRUE(result);

  erd_cache_entry_t* entry = find_entry(&cache, 0x0020, 0xFF);
  CHECK(NULL != entry);
...
TEST(erd_cache, insert_large)
{
  ...
  bool result = erd_cache_update(&cache, 0x0100, 0xFF, data, 32);
  CHECK_TRUE(result);

  erd_cache_entry_t* entry = find_entry(&cache, 0x0100, 0xFF);
  CHECK(NULL != entry);
...
TEST(erd_cache, update_existing_small)
{
  ...
  bool result = erd_cache_update(&cache, 0x0001, 0xFF, data2, 1);
  CHECK_TRUE(result);
...
TEST(erd_cache, update_existing_medium)
{
  ...
  erd_cache_update(&cache, 0x0020, 0xFF, data1, 5);
  bool result = erd_cache_update(&cache, 0x0020, 0xFF, data2, 5);
  CHECK_TRUE(result);
...
TEST(erd_cache, size_change_grows_returns_false)
{
  ...
  erd_cache_update(&cache, 0x0001, 0xFF, data1, 1);
  bool result = erd_cache_update(&cache, 0x0001, 0xFF, data2, 2);
  CHECK_FALSE(result);
}
...
TEST(erd_cache, size_change_shrinks_returns_false)
{
  ...
  uint8_t data2[] = { 0x01 };
  erd_cache_update(&cache, 0x0001, 0xFF, data1, 2);
  bool result = erd_cache_update(&cache, 0x0001, 0xFF, data2, 1);
  CHECK_FALSE(result);
}
...
TEST(erd_cache, get_next_updated_skips_non_updated)
{
  ...
  erd_cache_update(&cache, 0x0001, 0xFF, data1, 1);
  erd_cache_update(&cache, 0x0002, 0xFF, data2, 1);
  erd_cache_update(&cache, 0x0003, 0xFF, data3, 1);

  erd_cache_entry_t* e2 = find_entry(&cache, 0x0002, 0xFF);
  CHECK(NULL != e2);
...
TEST(erd_cache, required_update_rate_with_unchanged_data)
{
  ...
  erd_cache_update(&cache, 0x0001, 0xFF, data2, 1);
  CHECK_EQUAL(1, erd_cache_get_required_update_rate(&cache));
}
...
TEST(erd_cache, cache_full_rejects_new_erd)
{
  for (int i = 0; i < ERD_CACHE_CAPACITY; i++) {
    ...
    bool result = erd_cache_update(&cache, erd, 0xFF, data, 1);
    CHECK_TRUE(result);
  }
  CHECK_EQUAL(ERD_CACHE_CAPACITY, erd_cache_get_count(&cache));

  uint8_t data[] = { 0xFF };
  bool result = erd_cache_update(&cache, 0xFFFF, 0xFF, data, 1);
  CHECK_FALSE(result);
}
...
{
  uint8_t data[] = { 0x01, 0x02, 0x03, 0x04, 0x05 };
  erd_cache_update(&cache, 0x0020, 0xFF, data, 5);
...
TEST(erd_cache, rate_limit_multiple_erds_independent)
{
  ...
  uint8_t data3[] = { 0x03 };
  erd_cache_update(&cache, 0x0001, 0xFF, data3, 1);
  ...
  uint8_t data4[] = { 0x04 };
  erd_cache_update(&cache, 0x0002, 0xFF, data4, 1);
  iter = 0;
  CHECK(NULL == erd_cache_get_next_updated(&cache, &iter));
}
...
TEST(erd_cache, insert_rejected_over_248_bytes)
{
  ...
  bool result = erd_cache_update(&cache, 0x0100, 0xFF, data, 249);
  CHECK_FALSE(result);
  CHECK_EQUAL(0, erd_cache_get_count(&cache));
}
...
TEST(erd_cache, insert_accepted_at_248_bytes)
{
  ...
  bool result = erd_cache_update(&cache, 0x0100, 0xFF, data, 248);
  CHECK_TRUE(result);
  CHECK_EQUAL(1, erd_cache_get_count(&cache));
...
TEST(erd_cache, arena_full_rejects_new_erd)
{
  ...
  for (uint16_t i = 0; i < entries_to_fill; i++) {
    ...
    tiny_erd_t erd = (tiny_erd_t)(0x0001 + i);
    bool result = erd_cache_update(&cache, erd, 0xFF, data, entry_size);
    CHECK_TRUE(result);
  }
  ...
  bool result = erd_cache_update(&cache, 0xFFFF, 0xFF, data, entry_size);
  CHECK_FALSE(result);
  CHECK_EQUAL(entries_to_fill, erd_cache_get_count(&cache));
}
...
TEST(erd_cache, arena_usage_tracking)
{
  ...
  uint8_t data[100];
  memset(data, 0xAA, sizeof(data));
  erd_cache_update(&cache, 0x0001, 0xFF, data, 100);
  ...
  erd_cache_update(&cache, 0x0002, 0xFF, data2, 50);
...
/* ------------------------------------------------------------------ */
/* Multi-board address tests                                           */
/* ------------------------------------------------------------------ */

TEST(erd_cache, same_erd_different_addresses_create_separate_entries)
{
  uint8_t data1[] = { 0xAA };
  uint8_t data2[] = { 0xBB };
  CHECK_TRUE(erd_cache_update(&cache, 0x1000, 0xFF, data1, 1));
  CHECK_TRUE(erd_cache_update(&cache, 0x1000, 0x10, data2, 1));

  CHECK_EQUAL(2, erd_cache_get_count(&cache));

  erd_cache_entry_t* e1 = find_entry(&cache, 0x1000, 0xFF);
  erd_cache_entry_t* e2 = find_entry(&cache, 0x1000, 0x10);
  CHECK(NULL != e1);
  CHECK(NULL != e2);
  CHECK(e1 != e2);

  const uint8_t* d1 = erd_cache_entry_data(&cache, e1);
  const uint8_t* d2 = erd_cache_entry_data(&cache, e2);
  CHECK_EQUAL(0xAA, d1[0]);
  CHECK_EQUAL(0xBB, d2[0]);
}

TEST(erd_cache, find_entry_distinguishes_by_board_address)
{
  uint8_t data[] = { 0xCC };
  CHECK_TRUE(erd_cache_update(&cache, 0x2000, 0x20, data, 1));

  erd_cache_entry_t* found = find_entry(&cache, 0x2000, 0x20);
  CHECK(NULL != found);

  erd_cache_entry_t* not_found = find_entry(&cache, 0x2000, 0xFF);
  CHECK(NULL == not_found);
}

TEST(erd_cache, update_existing_entry_by_erd_and_address)
{
  uint8_t data1[] = { 0x01 };
  uint8_t data2[] = { 0x02 };
  CHECK_TRUE(erd_cache_update(&cache, 0x3000, 0x10, data1, 1));
  CHECK_TRUE(erd_cache_update(&cache, 0x3000, 0x10, data2, 1));
  CHECK_TRUE(erd_cache_update(&cache, 0x3000, 0x20, data2, 1));

  CHECK_EQUAL(2, erd_cache_get_count(&cache));

  erd_cache_entry_t* e1 = find_entry(&cache, 0x3000, 0x10);
  erd_cache_entry_t* e2 = find_entry(&cache, 0x3000, 0x20);
  const uint8_t* d1 = erd_cache_entry_data(&cache, e1);
  const uint8_t* d2 = erd_cache_entry_data(&cache, e2);
