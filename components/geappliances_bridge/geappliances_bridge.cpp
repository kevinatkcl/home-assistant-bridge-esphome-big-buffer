...
bool GeappliancesBridge::process_jsonl_line_(const char* line,
  ...
  if (strcmp(domain, "sensor") == 0) {
    ...
    // Read min/max/step from JSONL.  These fields are always present in
    // up-to-date JSONL files (generated by generate_erd_lists.py).
    ...
    double min_val  = (mn_item && cJSON_IsNumber(mn_item)) ? mn_item->valuedouble  :   0.0;
    double max_val  = (mx_item && cJSON_IsNumber(mx_item)) ? mx_item->valuedouble  : 100.0;
    double step_val = (st_item && cJSON_IsNumber(st_item)) ? st_item->valuedouble  :   1.0;
